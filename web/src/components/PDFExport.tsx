...
import { FileDown, Loader2, Lock, FileText, Image as ImageIcon } from 'lucide-react';
...
export function PDFExport({ sessionId, sessionData }: PDFExportProps) {
  ...
  const handleExportFormal = () => {
    ...
    try {
      ...
      doc.text(`Regulation Framework: ${(regulation || 'STANDARD').toUpperCase()}`, 14, 38);
      ...
      const riskLevel = session?.analysis_results?.overall_risk_level || 'UNKNOWN';
      ...
      if (tableData.length === 0) tableData.push(['N/A', 'No metrics found', '-', '-']);
      ...
      if (explanations && Object.keys(explanations).length > 0) {
        ...
        autoTable(doc, {
          startY: finalY + 5,
          head: [['Attribute', 'Metric', 'Summary', 'Recommendation', 'Severity']],
          body: expData,
          theme: 'grid',
          headStyles: { fillColor: [10, 10, 10], textColor: 255 },
      ...
      if (intersectional && intersectional.intersections && intersectional.intersections.length > 0) {
        if (finalY > 250) { doc.addPage(); finalY = 20; }
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        ...
        if (intersectional.most_disadvantaged) {
          doc.setFontSize(10);
          doc.setFont('helvetica', 'normal');
          doc.text(`Most Disadvantaged Group: ${intersectional.most_disadvantaged.group} (${intersectional.most_disadvantaged.intersection})`, 14, finalY + 8);
          doc.text(`Predicted Positive Rate: ${(intersectional.most_disadvantaged.predicted_positive_rate * 100).toFixed(1)}% (n=${intersectional.most_disadvantaged.count})`, 14, finalY + 13);
          finalY += 18;
        } else {
          finalY += 5;
        }
        ...
        intersectional.intersections.forEach((int: any) => {
          Object.entries(int.metrics || {}).forEach(([mName, mData]: [string, any]) => {
            intData.push([
      ...
      if (attribution && attribution.bias_drivers && attribution.bias_drivers.length > 0) {
        if (finalY > 250) { doc.addPage(); finalY = 20; }
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
